...
use reqwest::Url;
use sindri::{
    client::SindriClient, integrations::sp1_v4::SP1ProofInfo, ProofInput as sindriProofInput,
};
...
impl SP1HeliosOperator {
    ...
    async fn request_update(
        ...
        let project_team = "zzzz";
        let project_name = "sindri-sp1-helios";
        ...
        // Generate proof using sp1 client
        // let proof = self.client.prove(&self.pk, &stdin).groth16().run()?;

        let proof_input = sindriProofInput::try_from(stdin)?;
        ...
        let sp1_proof: SP1ProofWithPublicValues = proof_info.to_sp1_proof_with_public().unwrap();
        let sindri_verifying_key = proof_info.get_sp1_verifying_key().unwrap();
    ...
    async fn relay_update(&self, proof: SP1ProofWithPublicValues) -> Result<()> {
        ...
        let receipt = contract
            ...
            .get_receipt()
            .await?;
        // TODO: fix ProofInvalid() error 0x7fcdd1f4
